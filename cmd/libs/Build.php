...
class Build {
  ...
  public function searchHtml ($title) {
    $objs = array_merge (array_map (function ($t) {
      return array ('g' => '開發心得', 'h' => PAGE_URL_DEVS, 't' => $t['title'], 'c' => mb_strimwidth (removeHtmlTag ($t['content']), 0, 300, '…','UTF-8'), 'm' => $t['icon']['c300x300'], 'l' => $t['_url'], 'a' => $t['tag'], 's' => array_values (columnArray ($t['tags'], 'name')));
    }, $this->apis['devs']), array_map (function ($t) {
      return array ('g' => '生活紀錄', 'h' => PAGE_URL_BLOGS, 't' => $t['title'], 'c' => mb_strimwidth (removeHtmlTag ($t['content']), 0, 300, '…','UTF-8'), 'm' => $t['icon']['c300x300'], 'l' => $t['_url'], 'a' => $t['tag'], 's' => array_values (columnArray ($t['tags'], 'name')));
    }, $this->apis['blogs']), array_map (function ($t) {
      return array ('g' => '開箱文章', 'h' => PAGE_URL_UNBOXINGS, 't' => $t['title'], 'c' => mb_strimwidth (removeHtmlTag ($t['content']), 0, 300, '…','UTF-8'), 'm' => $t['icon']['c300x300'], 'l' => $t['_url'], 'a' => $t['tag'], 's' => array_values (columnArray ($t['tags'], 'name')));
    }, $this->apis['unboxings']), array_map (function ($t) {
      return array ('g' => '個人相簿', 'h' => PAGE_URL_ALBUMS, 't' => $t['title'], 'c' => mb_strimwidth (removeHtmlTag ($t['content']), 0, 300, '…','UTF-8'), 'm' => $t['cover']['c630x315'], 'l' => $t['_url'], 'a' => $t['tag'], 's' => array_values (columnArray ($t['tags'], 'name')));
    }, $this->apis['albums']));
